...
개발자가 ```new Example(b,a)```로 해도 전혀 문제를 찾을수가 없습니다.  
...
Posts 클래스가 실제로 DB에 잘 저장되는지 확인하는 테스트 코드를 추가하겠습니다.
...
public class PostsRepositoryTest {
    ...
    public void cleanup() {
        ...
        테스트 메소드가 끝날때 마다 respository 전체 비움
        **/
...
스프링프레임워크에선 Bean 을 주입받는 여러 방식이 있는데요.  
...
실제로 Controller에서 사용할 DTO 클래스를 생성하겠습니다.
...
별도로 입력화면이 없기 때문에 [Postman](https://chrome.google.com/webstore/detail/postman/fhbjgbiflinjbdggehcddcbncdddomop)을 통해 검증하겠습니다.  
  
Application.java를 다시 실행하신뒤, 브라우저에서 localhost:8080/h2-console
기존에 테스트 코드를 사용하지 않을때는 DB에 데이터가 잘 전달되는지 확인하기 위해 **입력화면, Controller, Serivce, 로컬 DB까지 전부 구성**되어야했다는 것입니다.  
이 모든게 다 갖춰줘야만 확인할 수 있다면 어느 세월에 가능할까요?  
